resource "azurerm_log_analytics_workspace" "log_analytics_workspace" {
  name                = "log-main-load-test-workspace-${local.suffix}"
...
resource "azurerm_application_insights" "main_application_insights" {
  ...
  location            = var.location
  name                = "appi-mainfuncapp-appinsights-${local.suffix}"
  resource_group_name = azurerm_resource_group.main_resource_group.name
  workspace_id        = azurerm_log_analytics_workspace.log_analytics_workspace.id
}

resource "azurerm_role_assignment" "appi_role_assignment_reader_load_test_identity" {
  scope                = azurerm_application_insights.main_application_insights.id

resource "azurerm_storage_account" "main_storage_account" {
  ...
  name                            = "samainaccloadtest${local.suffix}"
...
resource "azurerm_storage_container" "enriched_events_container" {
...
resource "azurerm_role_assignment" "storage_account_contributor" {
  scope                = azurerm_storage_container.enriched_events_container.id

resource "azurerm_eventhub_namespace" "main_eventhub_namespace" {
  name                          = "evh-test-main-eventhub-ns-${local.suffix}"
...
resource "azurerm_role_assignment" "evh_role_assignment_data_receiver_func_identity" {
  scope                = azurerm_eventhub.main_eventhub_topic.id
  role_definition_name = "Azure Event Hubs Data Receiver"
  principal_id         = azurerm_user_assigned_identity.main_func_identity.principal_id
}

resource "azurerm_role_assignment" "evh_role_assignment_data_sender_load_test_identity" {
  scope                = azurerm_eventhub.main_eventhub_topic.id
  role_definition_name = "Azure Event Hubs Data Sender"
  principal_id         = azurerm_user_assigned_identity.main_load_test_identity.principal_id
}

resource "azurerm_role_assignment" "evh_role_assignment_reader_load_test_identity" {
  scope                = azurerm_eventhub_namespace.main_eventhub_namespace.id
